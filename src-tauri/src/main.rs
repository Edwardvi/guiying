...
fn setup_native_runtime(app: &mut tauri::App, static_dir: PathBuf) -> Result<(), String> {
    ...
    let metadata_path = app
        .path()
        .app_data_dir()
        .map_err(|error| format!("Cannot resolve 桂英 app data directory: {error}"))?
    ...
    log::info!(
        "[guiying-native] started workspace_id={} session_id={} instance_id={} origin={}",
        target.workspace_id,
        target.session_id,
        target.instance_id,
        host.origin()
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            //      桂英 still running, or a previous `bun run dev` whose
            ...
            //
            // Allocating a fresh port for *this* 桂英 instance is the
            ...
            if initial_port != 47821 {
                log::warn!(
                    "[pi-desktop] port 47821 unavailable, using {} instead (likely another 桂英 instance is running)",
                    initial_port
                );
            }
