...
/// Hard guarantee: a 桂英 release build CANNOT be produced without the
...
fn main() {
    ...
    if !bin_path.is_file() {
        panic!(
            "\n\n\
             桂英 release build aborted: embedded pi binary is missing.\n\
             Expected: {}\n\n\
             桂英 bundles the pi runtime inside the .app so end users do\n\
    ...
    if !extension_bundle_path.is_file() {
        panic!(
            "\n\n\
             桂英 release build aborted: embedded-server extension bundle is missing.\n\
             Expected: {}\n\n\
