export function createAppUpdater({
  ...
  function syncSidebarUpdateButton() {
    ...
    if (pendingUpdate) {
      setSidebarUpdateButton({
        ...
        label: "Update",
        tone: "ok",
        title: `Download and install 桂英 ${pendingUpdate.version}`,
      });
      return;
    }
  ...
  function showInstallButton(update) {
    ...
    updateInstallLabel.textContent = `桂英 ${update.version}${from}`;
    ...
    try {
      ...
      setTimeout(() => {
        transport?.relaunchApp?.().catch((err) => {
          ...
          setUpdateStatus("Please restart 桂英 to finish updating.", "warn");
          updateCheckFailed = true;
          syncSidebarUpdateButton();
        });
      }, 600);
