export function setupSettingsEditors({
  ...
  function renderApiKeysPanel(providers) {
    ...
    // Only show opencode-go provider, renamed to "go"
    const filtered = providers
      .filter(p => p.provider === 'opencode-go')
      .map(p => ({ ...p, provider: 'go', displayName: 'go' }));
    for (const p of [...filtered].sort((a, b) => Number(b.configured) - Number(a.configured))) {
