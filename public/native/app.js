...
if (route.name !== "session") throw new Error("Native 桂英 requires a session route");
...
let commandCatalog = buildCommandCatalog({
  builtIns: [
    ...
    { name: "help", description: "Show 桂英 help", action: "show_help" },
  ],
});
...
async function loadBootstrapTarget(currentRoute) {
  ...
  if (!response.ok) throw new Error("This 桂英 runtime is stopped or unavailable");
